...
template<typename StringT = default_string_type>
struct pattern_lookup_data
{
public:
	...
	friend pattern_lookup_data<StringT> pattern_lookup(std::istream& in, SearcherT const& searcher);

private:
	QVector<std::tuple<size_t, size_t, StringT>> matches;
};
...
template<typename SearcherT, typename StringT = default_string_type>
pattern_lookup_data<StringT> pattern_lookup(std::istream& in, SearcherT const& searcher)
{
	std::array<char, default_max_line_size> buffer{};
	pattern_lookup_data<StringT> data;
	in.getline(buffer.data(), buffer.size());
	for (size_t line = 0; !in.eof() && !in.fail(); ++line, in.getline(buffer.data(), buffer.size()))
	{
		...
		if (count != 0) {
			data.matches.push_back({ count, line, StringT{ buffer.begin(), buffer.begin() + gcount } });
		}
	}
	return data;
}